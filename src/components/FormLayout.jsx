...
const FormLayout = () => {
  const { state: { currentStep, form }, handleInputChange, handleChangeMonth } = useStep()
  const { plans } = usePlan()
      ...
      {currentStep === 1 && (
        <div className='flex flex-col gap-3'>
          ...
          <label htmlFor='name' className='flex flex-col text-marine-blue font-medium'>
            Name
            <input
              onChange={handleInputChange} type='text' id='name' name='name' placeholder='e.g. Stephen King' className='border-light-gray border-2 rounded-md pl-2 py-2 text-sm' value={form.value}
            />
          </label>
          <label htmlFor='email' className='flex flex-col text-marine-blue font-medium'>
            Email Address
            <input onChange={handleInputChange} type='email' id='email' name='email' placeholder='e.g. stephenking@lorem.com' className='border-light-gray border-2 rounded-md pl-2 py-2 text-sm' value={form.email} />
          </label>
          <label htmlFor='phone' className='flex flex-col text-marine-blue font-medium'>
            Phone Number
            <input onChange={handleInputChange} type='tel' id='phone' name='phone' placeholder='e.g. +52 1234567890' className='border-light-gray border-2 rounded-md pl-2 py-2 text-sm' value={form.phone} />
          </label>
